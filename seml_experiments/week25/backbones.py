...
def build_experiments(cfg):
    for ood_type, ood_type_short in (('left-out-classes', 'loc'), ('perturbations', 'per')):
        ...
        for edge_reco, edge_reco_suffix in ((False, '-no-reco'), (True, '-reco')):
            for spectral_norm, sn_suffix in ((False, '-no-sn'), (True, '-sn')):
                for model_type in ('gcn', 'appnp', 'gat', 'mlp', 'gcn_linear_classification'):
                    ...
                    if model_type == 'appnp':
                        subcfg['fixed']['model.appnp.diffusion_iterations'] = 16
                        subcfg['fixed']['model.appnp.teleportation_probability'] = .2
                    elif model_type == 'gat':
                        subcfg['fixed']['model.gat.num_heads'] = 8

                    if edge_reco:
                        subcfg['fixed']['model.reconstruction.reconstruction_type'] = 'energy'
                        subcfg['fixed']['model.reconstruction.loss_weight'] = 1.0

                    if spectral_norm:
                        subcfg['fixed']['model.use_spectral_norm'] = spectral_norm
                        subcfg['fixed']['model.weight_scale'] = 200.0
                        subcfg['fixed']['model.residual'] = True
                    
                    subcfg['fixed']['run.name'] = 'model:{1}-setting:{0}-dataset:{2}' + edge_reco_suffix + sn_suffix
                    subcfg['fixed']['data.ood_type'] = ood_type
                    subcfg['fixed']['model.model_type'] = model_type

                    key = f'{ood_type}-{model_type}-{edge_reco_suffix}-{sn_suffix}'
